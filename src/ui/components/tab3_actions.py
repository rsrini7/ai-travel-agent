...
def handle_vendor_reply_submit(active_enquiry_id_tab3: str, vendor_reply_text_input: str):
    ...
    with st.spinner("Saving vendor reply..."):
        ...
        if reply_data:
            ...
            st.session_state.app_state.tab3_vendor_reply_info = {'text': reply_data['reply_text'], 'id': reply_data['id']}
            # Invalidate graph cache and clear outputs as vendor reply changed
            st.session_state.app_state.tab3_cached_graph_output = None
            st.session_state.app_state.tab3_cache_key = None
            st.session_state.app_state.tab3_quotation_pdf_bytes = None
            st.session_state.app_state.tab3_quotation_docx_bytes = None
            st.session_state.app_state.show_quotation_success_tab3 = False
            # Reset current quotation DB record info as it's tied to the previous vendor reply
            st.session_state.app_state.tab3_current_quotation_db_id = None
            st.session_state.app_state.tab3_current_pdf_storage_path = None
            st.session_state.app_state.tab3_current_docx_storage_path = None
            st.rerun()
        else:
...
def _handle_pdf_processing_and_storage(
    ...
    st.session_state.app_state.tab3_quotation_pdf_bytes = pdf_bytes_output
    ...
    if pdf_bytes_output and not is_error_content_in_pdf and len(pdf_bytes_output) > 1000: # Basic validity check for actual content
        ...
        if upload_err:
            ...
            st.session_state.app_state.tab3_current_pdf_storage_path = storage_path_for_db
        ...
        with st.spinner("Saving PDF quotation data to database..."):
            ...
            q_data, q_error = add_quotation(
                ...
                itinerary_used_id=st.session_state.app_state.tab3_itinerary_info.get('id'),
                vendor_reply_used_id=st.session_state.app_state.tab3_vendor_reply_info.get('id'),
            ...
            if q_error:
                ...
                st.session_state.app_state.tab3_current_quotation_db_id = q_data['id']
                if not upload_err: 
                ...
                st.session_state.app_state.show_quotation_success_tab3 = True
                st.rerun() 
    else:
        ...
        st.session_state.app_state.show_quotation_success_tab3 = False # No full success
...
def _handle_docx_processing_and_storage(
    ...
    st.session_state.app_state.tab3_quotation_docx_bytes = docx_bytes_for_upload
    ...
    if docx_bytes_for_upload: # DOCX conversion was successful
        ...
        if up_err_docx:
            ...
            st.session_state.app_state.tab3_current_docx_storage_path = storage_path_for_db_docx
        
        with st.spinner("Saving/updating DOCX quotation data..."):
            existing_quotation_id = st.session_state.app_state.get('tab3_current_quotation_db_id')
            ...
            if existing_quotation_id: # A PDF was likely generated first and record exists
                ...
                q_data_dx, q_err_dx = add_quotation(
                    ...
                    st.session_state.app_state.tab3_itinerary_info.get('id'),
                    st.session_state.app_state.tab3_vendor_reply_info.get('id'),
                    pdf_storage_path=st.session_state.app_state.get('tab3_current_pdf_storage_path'), 
                    docx_storage_path=storage_path_for_db_docx
                )
                if q_err_dx:
                    st.error(f"Failed to save new DOCX quotation data: {q_err_dx}")
                else:
                    st.session_state.app_state.tab3_current_quotation_db_id = q_data_dx['id']
                    if not up_err_docx:
...
def _get_or_generate_quotation_graph_data(current_graph_cache_key: str) -> tuple[bytes | None, dict | None, bool]:
    ...
    Returns:
        tuple: (pdf_bytes, structured_data, has_critical_error)
               ...
               If pdf_bytes are returned, they are also stored in st.session_state.app_state.tab3_quotation_pdf_bytes.
    """
    if st.session_state.app_state.tab3_cached_graph_output and st.session_state.app_state.tab3_cache_key == current_graph_cache_key:
        st.info("Using cached data for quotation generation.")
        pdf_bytes, structured_data = st.session_state.app_state.tab3_cached_graph_output
        st.session_state.app_state.tab3_quotation_pdf_bytes = pdf_bytes 
        return pdf_bytes, structured_data, False 

    itinerary_text_for_graph = st.session_state.app_state.tab3_itinerary_info.get('text', "Itinerary suggestions not available.")
    current_enquiry_details_for_gen = st.session_state.app_state.tab3_enquiry_details.copy()
    current_enquiry_details_for_gen["client_name_actual"] = st.session_state.app_state.tab3_client_name
    provider_for_generation = st.session_state.app_state.selected_ai_provider
    ...
    with st.spinner(f"Generating quotation data with {provider_for_generation}..."):
        pdf_bytes_output, structured_data_dict = run_quotation_generation_graph(
            ...
            st.session_state.app_state.tab3_vendor_reply_info['text'],
    ...
    if not has_critical_error:
        st.session_state.app_state.tab3_cached_graph_output = (pdf_bytes_output, structured_data_dict)
        st.session_state.app_state.tab3_cache_key = current_graph_cache_key
...
def handle_pdf_generation(active_enquiry_id_tab3: str, current_graph_cache_key: str):
    ...
    st.session_state.app_state.tab3_quotation_docx_bytes = None 
    st.session_state.app_state.tab3_current_docx_storage_path = None
    st.session_state.app_state.tab3_current_quotation_db_id = None # PDF generation creates a new quotation record
    st.session_state.app_state.show_quotation_success_tab3 = False
    ...
    if has_critical_error:
        ...
        # Error messages already shown. Error PDF (if any) is in st.session_state.app_state.tab3_quotation_pdf_bytes
        return
...
def handle_docx_generation(active_enquiry_id_tab3: str, current_graph_cache_key: str):
    ...
    if has_critical_error:
        ...
        # Error PDF (if any) is in st.session_state.app_state.tab3_quotation_pdf_bytes via the helper
        return
