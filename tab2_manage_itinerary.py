...
def render_tab2():
    ...
    # RENAMED: Display one-time messages if set
    if st.session_state.get('operation_success_message'):
        st.success(st.session_state.operation_success_message)
        st.session_state.operation_success_message = None # Clear after displaying
                    ...
                    with st.spinner(f"Generating AI suggestions with {st.session_state.selected_ai_provider}..."):
                        ...
                        if "Error:" not in suggestions_text and "Critical error" not in suggestions_text and "OpenRouter Model Error:" not in suggestions_text:
                            ...
                            if new_suggestion_record:
                                ...
                                st.session_state.itinerary_loaded_for_tab2 = enquiry_id_tab2
                                # RENAMED: Set the success message
                                st.session_state.operation_success_message = "AI Place suggestions generated and saved successfully!"
                                st.rerun()
                            else:
